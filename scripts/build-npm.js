...
async function downloadNpm(platform) {
  if (!platform || platform === 'mac') {
    ...
    await downloadNpmPackage('@libsql/darwin-x64', 'https://registry.npmjs.org/@libsql/darwin-x64/-/darwin-x64-0.4.7.tgz')
  }
...
downloadNpm(platformArg).catch(error => {
  console.error('Error downloading npm packages:', error)
  process.exit(1)
})
